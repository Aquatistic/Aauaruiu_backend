...
public interface MeasurementRepository extends JpaRepository<Measurement, Long> {
  ...
  @Query(
      "Select mes from Measurement mes where mes.userSensor.userSensorId= :userSensorId order by"
          + " mes.measurementTimestamp desc FETCH NEXT :count ROWS ONLY")
  public List<Measurement> findLastMeasurements(Long userSensorId, Integer count);

  @Query(
      "Select mes from Measurement mes where mes.userSensor.userSensorId= :userSensorId and"
          + " EXTRACT(day from mes.measurementTimestamp) > EXTRACT(day from LOCALTIMESTAMP) - :days"
          + " order by mes.measurementTimestamp desc")
