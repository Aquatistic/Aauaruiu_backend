...
public class EffectorControllController {
  ...
  public ResponseEntity<List<EffectorControll>> getEffectorControls(@PathVariable Long effectorId) {
    return new ResponseEntity<>(
        effectorControllService.getEffectorControlsByEffectorId(effectorId), HttpStatus.OK);
  }

  @GetMapping("/current/{effectorId}")
  public ResponseEntity<EffectorControll> getCurrentEffectorControls(
      @PathVariable Long effectorId) {
    return new ResponseEntity<>(
        effectorControllService.getCurrentEffectorControlsByEffectorId(effectorId), HttpStatus.OK);
  }

  @GetMapping("/future/{effectorId}/{daysInFuture}")
  public ResponseEntity<List<EffectorControll>> getFutureEffectorControls(
      @PathVariable Long effectorId, @PathVariable int daysInFuture) {
    return new ResponseEntity<>(
